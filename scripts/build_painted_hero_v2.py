"""Baut den gemalten Acht-Richtungs-Helden mit 16 Frames je Richtung."""
...
def build_proofs(rows: list[list[Image.Image]]) -> dict[str, str]:
    ...
    grid = Image.new("RGBA", (CELL * 16, CELL * 8 + 24), DARK)
    draw = ImageDraw.Draw(grid)
    draw.text(
        (5, 5),
        "Painted V2: Stand | Gehen x8 | Schwert x6 | Block - Richtungen S, SW, W, NW, N, NE, E, SE",
    ...
    save_gif([direction_canvas(rows, frame) for frame in range(1, 9)], walk_path, TIMING_MS["walk"])

    attack_path = proofs / "proof-v2-attack-8dir.gif"
    save_gif(
        [direction_canvas(rows, frame) for frame in range(9, 15)],
        attack_path,
        TIMING_MS["attack"],
    )
...
def main() -> None:
    ...
    source_rows = [source_cells(direction) for direction in DIRECTIONS]
    scale = global_scale(source_rows)
    rows = [normalize(cells, scale) for cells in source_rows]
    sheet = assemble(rows)
    sheet_name = "aldric-abenteurer-schwert-painted-16x8.png"
