...
- F6 (Dok 06 Teil H, Balance-Pass - gebaut + verifiziert): (1) GOLEM-ELITE als BINDUNGS-Mechanik statt Held-Nerf: ungebunden prallt fast alles am Panzer ab (15% Schaden kommt durch), erst wenn 3+ Nahkaempfer (Truppen/Held, Radius 110px) ihn gleichzeitig bedraengen, nimmt er 60% - der Held bleibt stark, aber den Koloss faellt nur die ARMEE mit ihm zusammen. Sein Rueckstoss streut die Binder wieder (gewolltes Spiel: neu binden!). (2) ANGRIFFS-SLOTS BEIDSEITIG: Ring-Plaetze gibt es jetzt um JEDES Ziel (je Ziel gruppiert) - Monster kreisen einen Soldaten ein, Soldaten einen Golem, nicht mehr nur den Helden. (3) WELLEN-DECKEL: der endlose F5-Sturm-Nachschub kommt nur, solange weniger als 16 Feinde leben (nie Hunderterhorden). (4) FORMATIONS-ANGRIFF: die Feldzug-Welle marschiert in Reihen zu 5 und haelt am Ziel eine BREITE FRONT (gestreute Zielpunkte) statt auf einen Punkt zu klumpen. (5) BLUTLAGER-COMEBACK: faellt ein Feindlager, geben die uebrigen Lager die Haelfte ihrer gesparten Punkte ab und die Produktion laeuft 150s auf 35% - der Schwaechere bekommt Luft (Dok 06 B). Alle Werte in FELDZUG (src/data/welt.ts).
- KI-TEIL-2 (24-Punkte-Dokument des Autors, gesichtet + entschieden): vollstaendige Entscheidungs-Tabelle in docs/design/07-FEIND-KI.md TEIL 2. UMGESETZT: (D1) Spaeher liefern eine SCHAETZUNG (+-25% Unschaerfe) statt des Exaktwerts - keine allwissende KI; (D2) Wechselhuerde: nach 3 vergeblichen Spaeh-Runden (Ziel zu stark verstaerkt) gibt der Feind das Ziel auf und plant neu, statt sich festzubeissen; (D3) Stuck-Detection (R166-Entklemmer) wacht jetzt auch ueber Feldzug-Wellen auf JEDER Karte; (D4) Zielbudgets/Angriffsplaetze sind mit F6-Slots abgedeckt. NICHT uebernommen (mit Begruendung im Dokument): Reservierungs-Besitzer, TrackedEnemyForce/Sammlungs-Score, Detachment-Baum/Anforderungsprofile (CAVALRY/SIEGE verstossen zudem gegen Regeln 5/6), Korridor/Sync/Flanken/Frontabschnitte (Mehr-Gruppen-Buehne fehlt bewusst - welleMax), Feind-Rueckzug mit Nachhut (kollidiert mit R147b: Untote fliehen nicht), freie Bau-KI (kollidiert mit A9/F3), Kurz-Kampfsimulation (unsere abstrakte Aufloesung IST sie).
- KI-TEIL-2 WEGFINDUNGS-PASS (Autor: "Kollision und Wegfindung... beachte das immer oder debugge"): DREI echte Funde behoben. (1) Marsch-Flussfelder gab es NUR im RTS-Modus (rtsBattle) - ohne ihn liefen Wellen/jagdZiel-Maersche per Luftlinie in die Baumwand. Jetzt hat die Welt-Szene ein eigenes Marschfeld (gleiche R188-Mechanik: freie Bahn -> direkt, sonst gecachtes Flussfeld) als Fallback. (2) Wellen-Formationen spawnten mit den hinteren Reihen IM Randbewuchs (Kanten-Punkt liegt nur 3 Kacheln vor dem Rand) - der Anker rueckt jetzt um die Reihentiefe ins Karteninnere, und jeder Platz wird auf freien UND angeschlossenen Boden geprueft. (3) Der Entklemmer greift auf allen Karten. VERIFIZIERT: Stadt 10/10 Marschierer sauber durch (958-1889px, 0 Haenger); Waldkarte von 7 auf 3 Rand-Taschen-Starter reduziert (die 3 faengt der Entklemmer, sie kaempfen normal weiter) - Rest-Ursache ist die fehlende kartenKanten-Verdrahtung (Wellen muessten auf der STRASSE spawnen), siehe TODO.
- FEINDLAGER-MITTELWEG M1 (Autor bestaetigt "so machen wir das", gebaut+verifiziert): das Feindlager baut nicht mehr nach EINER festen Form, sondern nach VORGEFERTIGTEN routen-sicheren BLAUPAUSEN (FEINDLAGER_VARIANTEN in data/welt.ts): Sichel/Hufeisen/Zwei Riegel/Voller Ring, je Wall-Form + Radius + Tor-Breite + Waechter-Ring + Altar-Versatz. Ein Seed je Karten-Name waehlt stabil eine Variante (dieselbe Karte immer gleich, verschiedene Karten verschieden). JEDE Form laesst grosse Oeffnungen (wallHatWand-Logik) - nie ein geschlossener Kasten. Verifiziert im Browser: 4 Karten -> 3 verschiedene Varianten; Wall wird real gebaut (11-24 CRACK-Kacheln je Form) und ist solide; Altar bleibt auf ALLEN Karten vom Rand per BFS erreichbar (keine Karte zugebaut, Autor-Order). NAECHSTE Schritte (M2): Garnison sortiert aus statt eingesperrt + Lager auf der Route; Monster-Arbeiter + Optik spaeter. Oekonomie bleibt abstrakt (M3-Empfehlung, keine Minen-Wirtschaft) - Zeit-Gate wie F3.
- STRESS-MESSUNG Einheiten-Grenze (Autor "messe 1000/2000, wo ist die Grenze"): LOGIK skaliert mild (300->10ms, 600->9ms, 1000->12ms, 2000->21ms Median, headless-CPU) - die KI/Kollision ist NICHT die Grenze. Das WebGL-Rendering ist headless NICHT verlaesslich messbar (SwiftShader-Software-Render, ~720ms KONSTANT ueber 300..2000 = Vollbild-Passes, nicht die Einheiten). Echte Grenze = GPU + GC, nur auf echter Hardware messbar. Darum DEV-Stresstest (Taste B: 300/600/1000/1500 Einheiten + Live-FPS; Shift+B raeumt) - der Autor misst die Grenze auf seinem PC. Reines Messwerkzeug, kein Spiel-Feature.
- R194 Fuerstenburg-Debug: Die sichtbare Burgkarte erzeugt keinen Wasser-Shader;
  die Fluss-Naht bleibt ausschliesslich als entfernte Randkachel/Kollision
  erhalten. Die Burg-Canvas wird am Spielerfuss in Hintergrund und Vordergrund
...
- R195 Fuerstenburg-Konstruktion: Das echte SM_Gate bleibt und wird um 90 Grad
  ...
  zur Wehrganghoehe.
- KOLLISIONS-OVERLAY (Autor-Bug "unsichtbare Wand, ich finde sie nicht" - Werkzeug + Diagnose): Taste K faerbt JEDE fuer den Helden blockierte Kachel farbcodiert (blau=Wasser, gruen=Baum-Fuss, rot=Fels/Erz, magenta=Gebaeude, cyan=Bruecke) + loggt die Verteilung. Diagnose auf 'start' (Waldrand, die Karte aus dem Autor-Screenshot): solide sind NUR Wasser(1281), Baum(175), Fels/Erz(52) - keine unbekannten Typen. Fels/Erz haben ALLE einen Eintrag (rendern als kleine Steine) -> NICHT die unsichtbare Wand. Verbleibende Ursache: BAUM-FUSSKOLLISION (Krone nach oben versetzt gezeichnet -> Stammfuss-Kachel sieht wie Gras aus, ist aber solid) ODER truebes Moorwasser das wie Boden liest. Autor loest final mit K (Farbe am Wand-Ort = Ursache), dann gezielter Fix.
- SCHADENSZAHLEN + TREFFERMELDUNGEN standardmaessig AUS (Autor: "Schadenszahlen und parieren/ausweichen kann grundsaetzlich aus sein, steht im Log; Lebensbalken will ich"): dmgNums-Default true->false; der Filter in effects.float schluckt jetzt Zahlen UND die Woerter Pariert/Geblockt/Ausgewichen. Dramatische Texte (WUCHT, Spezialangriffe, Heilung) + LEBENSBALKEN bleiben. Umschalter "Schadenszahlen & Treffermeldungen" in den Einstellungen. Nebennutzen: weniger Text-Objekte = Perf in Massenschlachten.
- KOLLISIONS-KLICK-WERKZEUG (Autor "gib mir Koordinaten"): im K-Overlay zeigt ein Klick die Kachel (tx,ty) + Typ + GRUND (frei/WASSER/BAUM/WAND/GEBAEUDE/BRUECKE) im Log + oben. Verifiziert: Wasser-Klick -> "WASSER (Fluss?)", Baum-Klick -> "BAUM". Autor klickt die unsichtbare Wand an, nennt Koordinaten+Grund; sagt der Grund WASSER wo Gras zu sehen ist, ist es die alte Fluss-Kollision (R185-Umleitung), dann gezielter recarve-Fix.
- R196 Fuerstenburg-Feinpass: Keine aufeinander gestapelten Treppenmodule und
  keine rein dekorativen, schwebenden Podeste. Jeder Wehrgangaufgang ist ein
  einzelner 6,4-m-Lauf vom Hofboden bis 4,13 m. Mauermodule duerfen fuer eine
  robuste Silhouette 28 cm ueberlappen; gerade Naehte bekommen echte
  SM_CastleWallSupport-Teile. Eckverbindungen sind vollhohe, exakt mauerstarke
...
- 2026-07-22: Das Lazarett folgt der Referenz als quadratisches Hochdachzelt mit Mittelmast, vier radialen Dachfeldern, Eckpfosten und umlaufender Zackenblende; ein umgefaerbtes oder skaliertes Firstzelt ist ausdruecklich keine zulaessige Naeherung. Aussenhaut bleibt `MAT_MEDICAL_TENT_CANVAS`.
