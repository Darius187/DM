...
export class Held3DModell {
  ...
  // facing = pdir; moving = läuft gerade; swingProg = 0..1 Schlag-Fortschritt (<0 = kein Schlag)
  update(dt: number, facing: number, moving: boolean, swingProg: number): void {
    if (!this.bereit) return;
    // Beine/Stand-Animation
    ...
    // Blickrichtung sanft nachführen
    ...
    // Schlag: ein Überkopf-Hieb nach vorn, an die echte Angriffszeit gekoppelt
    if (swingProg >= 0) {
      const e = easeOutQuart(Math.min(1, swingProg));
      // Horizontaler Hieb: die Klinge fegt von rechts (+1.15) nach links (-1.6)
      // quer über die Front - von oben gut als Schwung lesbar.
      this.schwertArm.rotation.y = 1.15 - e * 2.75;
      this.schwertArm.rotation.x = Math.sin(Math.min(1, swingProg) * Math.PI) * 0.35; // leichte Senke
      this.schwertArm.rotation.z = 0;
      // Funke im schnellsten Teil (Mitte des Schwungs)
