...
export function registriereWieseBitmaps(tex: Phaser.Textures.TextureManager): void {
  ...
  // LINEAR statt pixelArt-NEAREST: die gebackenen Bilder sind malerisch, nicht Pixel-Art.
  gras.forEach((cv, i) => { if (!tex.exists(`wiese_gras_${i}`)) tex.addCanvas(`wiese_gras_${i}`, cv)?.setFilter(Phaser.Textures.FilterMode.LINEAR); });
  blumen.forEach((cv, i) => { if (!tex.exists(`wiese_blume_${i}`)) tex.addCanvas(`wiese_blume_${i}`, cv)?.setFilter(Phaser.Textures.FilterMode.LINEAR); });
  bereit = true;
}
