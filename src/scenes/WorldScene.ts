  ...
  private baueDevTabs(): DKTab[] {
    ...
    return [
      ...
      { name: 'ANFANG', controls: () => {
        ...
        cs.push({ kind: 'button', label: () => `GEMALTER HELD (R247): ${getSettings().heldGemalt ? 'AN' : 'aus (Zeichnung)'}`, onClick: () => { getSettings().heldGemalt = !getSettings().heldGemalt; saveSettings(); this.devKonsole?.refresh(); } });
