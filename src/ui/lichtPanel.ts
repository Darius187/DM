...
export class LichtPanel {
  ...
  constructor(private scene: Phaser.Scene, x0: number, y0: number, opts?: { tageszeit?: TageszeitHaken; tiefe?: number }) {
    ...
    this.slider('Dungeon-Dunkelheit', 0, 150, () => getSettings().dungeonStaerke, (v) => { getSettings().dungeonStaerke = v; });
    ...
    // Autor: Held-Licht wahlweise VOR (Standard) oder HINTER dem Helden projizieren
    this.toggle(() => `Held-Licht Projektion: ${(L().heldLichtHinten ?? false) ? 'HINTER dem Helden' : 'vor dem Helden'}`, () => { L().heldLichtHinten = !(L().heldLichtHinten ?? false); });
